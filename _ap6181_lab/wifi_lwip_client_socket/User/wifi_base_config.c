...
void Config_WIFI_LwIP_Info()
{
    ...
    result = wwd_management_wifi_on( COUNTRY );//打开wifi超时
	
		if(result==2)
		{
			printf("打开wifi超时 ->>%d",result);
			while(1);
		}
    ...
    if ( NULL == netif_add( &g_wiced_if, &ipaddr, &netmask, &gw, (void*) WWD_STA_INTERFACE, ethernetif_init, ethernet_input ) )
    {
        WPRINT_APP_ERROR( ( "Failed to start network interface\n" ) );
        return;
    }

    netif_set_default( &g_wiced_if );
    netif_set_up( &g_wiced_if );

    /* 做DHCP协商*/
    WPRINT_APP_INFO(("Obtaining IP address via DHCP\n"));
    struct dhcp netif_dhcp;
    dhcp_set_struct( &g_wiced_if, &netif_dhcp );
    dhcp_start( &g_wiced_if );
    ...
    WPRINT_APP_INFO( ( "Network ready IP: %s\n", ip4addr_ntoa(netif_ip4_addr(&g_wiced_if))));

#if 0
    WPRINT_APP_INFO( ( "Shutting down WICED\n" ) );

    /* 再次关闭一切*/
    dhcp_stop( &g_wiced_if );
    netif_set_down( &g_wiced_if );
    netif_remove( &g_wiced_if );
